            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <div class="form-group d-flex align-items-center">
                                <button type="button" class="btn btn-primary" data-toggle="modal"
                                    onclick="openStockServiceModal('add')" data-target="#stockServiceModal">
                                    Ajouter
                                </button>
                            </div>
                            <form method="POST" enctype="multipart/form-data" id="importFile">
                                @csrf
                                <div class="form-group d-flex align-items-center">
                        ...
                        <x-stock-services.index :services="$services" />

                        <div class="card-body">
                            <div class="dt-bootstrap5 table-responsive-lg">
                                <table id="stockServicesTable" style="width:100%"
...
@section('scripts')
    ...
    <script>
        $('#reference_article').inputmask({
            mask: '9999-9999',
            greedy: false
        });
        let table = new DataTable('#stockServicesTable', {
            processing: true,
            // serverSide: true,
            ajax: "{{ route('stock-services.index') }}",
            columns: [{
                    data: 'service_id',
                    name: 'service_id'
                },
        ...
        function openStockServiceModal(action, id = null) {
            // console.log('Ouverture de la modal avec action :', action);
            // Réinitialiser le formulaire
            $('#stockServiceForm')[0].reset();

            // Modifier le titre de la modal en fonction de l'action
            $('#stockServiceModalLabel').text(action === 'add' ? 'Ajouter un materiel par service' :
                'Modifier un materiel par service');

            // Modifier l'action du formulaire en fonction de l'action
            $('#stockServiceForm').attr('action', action === 'add' ? '{{ route('stock-services.store') }}' :
                '{{ route('stock-services.update', ':id') }}'.replace(':id', id));

            // Pré-remplir les champs si l'action est 'edit'
            if (action === 'edit') {
                axios.get('{{ route('stock-services.edit', ':id') }}'.replace(':id', id))
                    .then(response => {
                        const service = response.data;
                        $('#service_id').val(service.service_id);
                        $('#designation').val(service.designation);
                        $('#reference_article').val(service.reference_article);
                        $('#stock_initial').val(service.stock_initial);
                        $('#entree').val(service.entree);
                        $('#sortie').val(service.sortie);
                        $('#stock_final').val(service.stock_final);
        ...
        function deleteStockService(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    axios.delete(`/stock-services/${id}`)
                        .then(response => {
        ...
        $('#importFile').submit((e) => {
            ...
            axios.post("/stock-services/import", formData)
                .then(response => {
                    Swal.fire({
                        icon: 'success',
        ...
        function saveStockService() {
            ...
            const action = $('#stockServiceForm').attr('action');

            // Effectuer la requête en fonction de l'action (ajout ou modification)
            axios({
                    method: action === '{{ route('stock-services.store') }}' ? 'post' : 'patch',
                    url: action,
                    data: $('#stockServiceForm').serialize(),
                })
                .then(response => {
                    handleServerResponse(response, 'Enregistré avec succès!', $('#stockServiceModal'));
                })
                .catch(error => {
                    // Gestion des erreurs d'Axios
                    console.error(error);
