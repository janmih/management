...
@section('scripts')
    ...
    <script>
        ...
        function openArticleModal(action, id = null) {
            // console.log('Ouverture de la modal avec action :', action);
            // Réinitialiser le formulaire
            $('#articleForm')[0].reset();

            // Modifier le titre de la modal en fonction de l'action
            $('#articleModalLabel').text(action === 'add' ? 'Ajouter un article' :
                'Modifier un article');

            // Modifier l'action du formulaire en fonction de l'action
            $('#articleForm').attr('action', action === 'add' ? '{{ route('articles.store') }}' :
                '{{ route('articles.update', ':id') }}'.replace(':id', id));

            // Pré-remplir les champs si l'action est 'edit'
            if (action === 'edit') {
                axios.get('{{ route('articles.edit', ':id') }}'.replace(':id', id))
                    .then(response => {
                        const service = response.data;
                        $('#service_id').val(service.service_id);
                        $('#reference_mouvement').val(service.reference_mouvement);
                        $('#compte_PCOP').val(service.compte_PCOP);
                        $('#reference').val(service.reference);
                        $('#designation').val(service.designation);
                        $('#conditionnement').val(service.conditionnement);
                        $('#unite').val(service.unite);
                        $('#date_peremption').val(service.date_peremption);
                        $('#provenance').val(service.provenance);
                        $('#entree').val(service.entree);
                        $('#etat_article').val(service.etat_article);
        ...
        function saveArticle() {
            ...
            const action = $('#articleForm').attr('action');

            // Effectuer la requête en fonction de l'action (ajout ou modification)
            axios({
                    method: action === '{{ route('articles.store') }}' ? 'post' : 'patch',
                    url: action,
                    data: $('#articleForm').serialize(),
                })
                .then(response => {
                    handleServerResponse(response, 'Enregistré avec succès!', $('#articleModal'));
                })
                .catch(error => {
                    // Gestion des erreurs d'Axios
                    console.error(error);
