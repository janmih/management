            ...
            return datatables()->of($personnel)
                ...
                ->addColumn('action', function ($row) {
                    if (Auth::user()->hasAnyRole('Ressource Humaine', 'Super Admin')) {
                        $btnEditer = '<button class="btn btn-warning btn-sm mb-3" onclick="openPersonnelModal(\'edit\', ' . $row->id . ')"><i class="fas fa-pencil"></i></button>';
                        $btnSupprimer = '<button class="btn btn-danger btn-sm mb-3" onclick="deletePersonnel(' . $row->id . ')"><i class="fas fa-trash"></i></button>';
