...
class CongePriseController extends Controller
{
     ...
     */
    public function getYears($personnelId)
    {
        // Récupère les années associées au personnel depuis la base de données
        $years = CongeCumule::where('personnel_id', $personnelId)->pluck('annee');
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getCongeRestante($personnelId, $annee)
    {
        // Récupère le congé restant associé au personnel et à l'année depuis la base de données
        $congeRestante = CongeCumule::where('personnel_id', $personnelId)
            ->where('annee', $annee)
            ->pluck('jour_reste');
